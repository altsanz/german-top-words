...
export function Question() {
    const [buttonColors, setButtonColors] = useState(['primary', 'primary', 'primary'])
    ...
    function selectOption(selectedOption) {
        const newColorButtons = question.possibleTranslations.map((option, index) => {
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12}>
                <TheWord word={question.germanWord}></TheWord>
            </Grid>
            {question.possibleTranslations.map((option, i) => (

                <Grid item xs={4}>
                    <Button variant="outlined" color={buttonColors[i]} onClick={() => selectOption(option)}>{option}</Button>
                </Grid>
            ))}
